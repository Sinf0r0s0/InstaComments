...
class InstaComments:
    ...
    def __init__(self, uri, max_com=49, timeout=10):
        self.uri = uri
    ...
    def re_search(self, tag_ini, tag_fin, input):
    ...
    def get_query_hash(self):
        ...
        r = self.rqst(f'https://www.instagram.com/p/{self.uri}/')
        if r:
            ppc = self.re_search(self.tag_to_re_a_i, self.tag_to_re_a_f, r.text)
            # print(ppc)
            rp = self.rqst(f'https://www.instagram.com/static/bundles/metro/ProfilePageContainer.js/{ppc}.js')
            if rp:
                # print(rp.text)
                return self.re_search(self.tag_to_re_b_i, self.tag_to_re_b_f, rp.text)
    ...
    def req_json(self, query_h, insta_uri, max_p, after=''):
        res = requests.get(
    ...
    def start(self, query_hash=None):
        ...
        if q_hash:
            if self.max_com > 0 <= 49:
                self.req_json(q_hash, self.uri, self.max_com)

            if all([self.max_com > 49, self.nova_pagina]):
                while len(self.list_response) < self.max_com:
                    self.req_json(q_hash, self.uri, self.max_com, self.nova_pagina)
